...
export default class Laser {
    ...
    createShot(x: number, y: number, xx: number, yy: number, speed: number, type: number, bullet?: Bullet) { // create a bullet object
        ...
        var nx = xx-x; // normalise // huh? normalise? the only normalisation I know is normalising a vector and from what I'm seeing, it isn't doing that. - zhich
        var ny = yy-y;
        var dist = Math.sqrt(nx*nx+ny*ny);
        nx /= dist; // oh, it is normalising the vector but a few lines later. - zhich
        ny /= dist;
        bullet.x = x;
        bullet.y = y;
        bullet.speed = speed;
    ...
    updateDrawAllBullets() {
        ...
        for (i = 0; i < this.bullets.length; i++) {
            ...
            if (b.life! <= 0) { // bullet end remove it and put it in the pool
                ...
                i--; // to stop from skipping a bullet // why aren't we just doing continue instead? - zhich
            } else {
                if (b.life! < 5) {
                    if (b.life === 4) {
...
type Bullet = {
    ...
    speed?: number;
    type?: number;
    xx?: number; // this is really unclear but, i think it's final position - zhich
    yy?: number;
    nx?: number;
    ny?: number;
    rot?: number; // bruh this is rotation and not rotting rate - zhich
    life?: number;
}
